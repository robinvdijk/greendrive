class Achievement < ActiveRecord::Base
  ...
  def self.getbadges
    ...
    for car in Car.all
      ...
      for badge_tutorial in Badge.where('id not in (select badge_id from achievements where user_id = ?) AND subject = ? and value <= ? ', car.user.id, 'tutorial', car.mileage)
        car.user.badges << badge_tutorial
      end
